...
export const NavbarLogo = () => {
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('site_settings').select('value').eq('key', 'logo').single();
      if (error) {
  ...
  return <Link to="/" className="flex items-center gap-2 flex-shrink-0">
      <img src={logoSettings?.url || '/placeholder.svg'} alt="TRADECOINER" onError={e => {
